...
query = 'Monster Moon\'s home'
query = iconv(query, to = 'UTF-8', toRaw = T)[[1]] %>% 
  paste(., collapse = '%') %>% paste0('%', .) %>%
  toupper()

map_api_url = paste0('https://openapi.naver.com/v1/map/geocode.xml?query=', query)
map_api_url_get = GET(map_api_url, header)
if(map_api_url_get$status_code == 200) print('Great !')
geocode_locat = read_xml(map_api_url_get) %>% xml_nodes('items item point') %>% 
